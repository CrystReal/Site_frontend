...
class Forum extends CActiveRecord
{
    ...
    public function rules()
    {
        ...
        return array(
            ...
            array('isCat, listorder', 'numerical', 'integerOnly' => true),
    ...
    {
        return Yii::app()->createUrl('/forum/forum/index', array('id' => $this->id));
    }
    ...
    public function getBreadcrumbs($currentlink = false)
    {
        ...
        if (!$this->isNewRecord) {
            $breadcrumbs = array_merge($breadcrumbs, $currentlink
                    ? array(CHtml::encode($this->title) => array('/forum/forum/index', 'id' => $this->id))
...
<small>' . CHtml::link(AlexBond::time_since(time() - $lastpost->created), $thread->url) . '</small>
    ...
    public function getArrayForDropDownInThreads()
    {
        $out = [];
        $cats = $this->findAllByAttributes(["parent_id" => null]);
        foreach ($cats as $item) {
            $cat = [];
            foreach ($item->subforums as $forum) {
                $cat[$forum->id] = $forum->title;
            }
            $out[$item->title] = $cat;
        }
        return $out;
